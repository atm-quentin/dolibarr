...
if ($user->hasRight("holiday", "read")) {
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			$event->datep                   = $db->jdate($obj->date_start, 'gmt') + (int) ((empty($obj->halfday) || $obj->halfday == 1 ? 0 : 12) * 60 * 60);
			$event->datef                   = $db->jdate($obj->date_end, 'gmt') + (int) ((empty($obj->halfday) || $obj->halfday == -1 ? 24 : 12) * 60 * 60 - 1);
			$event->date_start_in_calendar  = $event->datep;
			$event->date_end_in_calendar    = $event->datef;
